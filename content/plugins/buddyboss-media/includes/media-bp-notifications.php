...
/**
 * Sends an email notification and a BP notification when someone posts on a wall
 *
 * @since BuddyBoss Media 1
 *
...
function bp_activity_at_message_notification( $activity_id, $receiver_user_id ) {
	...
	if ( 'no' != bp_get_user_meta( $receiver_user_id, 'notification_activity_new_mention', true ) ) {
		...
		$to       = $ud->user_email;
		$subject  = bp_get_email_subject( array( 'text' => sprintf( __( '%s mentioned you in an update', 'buddyboss-media' ), $poster_name ) ) );
...
---------------------
', 'buddyboss-media' ), $poster_name, bp_get_current_group_name(), $content, $message_link );
		} else {
			$message = sprintf( __(
...
---------------------
', 'buddyboss-media' ), $poster_name, $content, $message_link );
		}

		// Only show the disable notifications line if the settings component is enabled
		if ( bp_is_active( 'settings' ) ) {
			$message .= sprintf( __( 'To disable these notifications please log in and go to: %s', 'buddyboss-media' ), $settings_link );
		}
...
function bp_activity_new_comment_notification( $comment_id, $commenter_id, $params ) {
	...
	if ( $original_activity->user_id != $commenter_id && 'no' != bp_get_user_meta( $original_activity->user_id, 'notification_activity_new_reply', true ) ) {
		...
		$to      = $ud->user_email;
		$subject = bp_get_email_subject( array( 'text' => sprintf( __( '%s replied to one of your updates', 'buddyboss-media' ), $poster_name ) ) );
		$message = sprintf( __(
...
---------------------
', 'buddyboss-media' ), $poster_name, $content, $thread_link );
	...
	if ( $parent_comment->user_id != $commenter_id && $original_activity->user_id != $parent_comment->user_id && 'no' != bp_get_user_meta( $parent_comment->user_id, 'notification_activity_new_reply', true ) ) {
		...
		$to       = $ud->user_email;
		$subject = bp_get_email_subject( array( 'text' => sprintf( __( '%s replied to one of your comments', 'buddyboss-media' ), $poster_name ) ) );
