...
window.onload = function () {
    ...
    var path = r.path().attr({stroke: color, "stroke-width": 4, "stroke-linejoin": "round"}),
        ...
        leave_timer,
        blanket = r.set();
    label.push(r.text(60, 12, "24 downloads").attr(txt));
    ...
    for (var i = 0, ii = labels.length; i < ii; i++) {
        ...
        (function (x, y, data, lbl, dot) {
            ...
            rect.hover(function () {
                ...
                label[0].attr({text: data + " download" + (data == 1 ? "" : "s")}).show().stop().animateWith(frame, {translation: [ppp.dx, ppp.dy]}, 200 * is_label_visible);
                label[1].attr({text: lbl + mmyy}).show().stop().animateWith(frame, {translation: [ppp.dx, ppp.dy]}, 200 * is_label_visible);
                dot.attr("r", 6);
