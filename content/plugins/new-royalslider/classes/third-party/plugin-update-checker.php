...
class RSPluginUpdateChecker_1_3 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile, $pluginData = null, $status = null) {
		if ( $pluginFile == $this->pluginFile && current_user_can('update_plugins') ) {
			...
			$linkUrl = wp_nonce_url(
				esc_url( add_query_arg('puc_check_for_updates', $this->slug, admin_url('plugins.php')) ),
	...
	public function handleManualCheck() {
		...
		if ( $shouldCheck ) {
			...
			wp_redirect( esc_url( add_query_arg('puc_update_check_result', $status, admin_url('plugins.php'))  ) );
		}
	}
