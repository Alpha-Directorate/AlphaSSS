...
class WPML_TM_Mail_Notification {
	...
	private function get_lang_to_from( $job, $user_language ) {
		...
		$lang_from = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT name FROM {$wpdb->prefix}icl_languages_translations WHERE language_code=%s AND display_language_code=%s",
	...
	private function enqueue_mail( $mail, $language = false ) {
		global $sitepress;
	...
	public function wpml_tm_job_complete_mail( $job_id, $update = false ) {
		...
		if ( $iclTranslationManagement->settings[ 'notification' ][ 'completed' ] == ICL_TM_NOTIFICATION_IMMEDIATELY ) {
			...
			if ( $update ) {
				...
				$mail[ 'body' ]    = sprintf(
					__(
						"Translator (%s) has updated translation of job \"%s\" for %s to %s.\n%s\n\nView translation jobs: %s",
						'sitepress'
					),
					$translator->display_name,
					$job->original_doc_title,
					$lang_from,
					$lang_to,
					$doc_url,
					$tj_url
				...
				$mail[ 'body' ]    = sprintf(
					__(
						"Translator (%s) has completed translation of job \"%s\" for %s to %s.\n%s\n\nView translation jobs: %s",
						'sitepress'
					),
					$translator->display_name,
					$job->original_doc_title,
	...
	public function new_job_translator_mail( $job_id, $translator_id ) {
		/** @var TranslationManagement $iclTranslationManagement */
		global $iclTranslationManagement, $sitepress;
		$job = $iclTranslationManagement->get_translation_job( $job_id );

		if ( !isset( $job->manager_id ) || !isset( $job->translator_id ) || $job->manager_id == $translator_id ) {
			return;
		}
	...
	public function translator_removed_mail( $translator_id, $job_id ) {
		/** @var TranslationManagement $iclTranslationManagement */
		global $iclTranslationManagement, $sitepress;
		$job = $iclTranslationManagement->get_translation_job( $job_id );
	...
	public function translator_resign_mail( $translator_id, $job_id ) {
		...
		if ( $iclTranslationManagement->settings[ 'notification' ][ 'resigned' ] == ICL_TM_NOTIFICATION_IMMEDIATELY ) {
			...
			$original_doc_title = isset( $job->original_doc_title )
				? $job->original_doc_title
				: __(
					"Deleted",
					"sitepress"
