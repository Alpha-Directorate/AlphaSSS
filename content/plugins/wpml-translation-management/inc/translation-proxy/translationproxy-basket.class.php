...
if ( ! class_exists( 'TranslationProxy_Basket' ) ) {
	...
	class TranslationProxy_Basket {
		...
		public static function sanitize_basket_name( $basket_name, $max_length ) {
			...
			if ( count( $basket_name_array ) < 3 ) {
				$wpml_flag = false;
				if ( count( $basket_name_array ) < 2 ) {
					return substr( $basket_name, $max_length - 1 );
				}
			}

			//Todo: [WPML 3.2.1] can't we make a proper function here?
			$shorten = create_function( '$basket_name_array_input, $index, $left_to_trim_', '
				if ( strlen( $basket_name_array_input [ $index ] ) > $left_to_trim_ ) {
					$basket_name_array_input[ $index ] = substr( $basket_name_array_input[ $index ], 0, strlen( $basket_name_array_input [ $index ] ) - $left_to_trim_ - 1 );
					$basket_name_array_input = array_filter( $basket_name_array_input );

					return array( $basket_name_array_input, 0 );
				} else {
					$left_to_trim_ = $left_to_trim_ - strlen( $basket_name_array_input [ $index ] ) - 1; //subtract one here since we lose a downstroke
					unset( $basket_name_array_input [ $index ] );

					return array( $basket_name_array_input, $left_to_trim_ );
				} ' );


			//first we trim the middle part holding the "WPML"
			if ( $wpml_flag ) {
				$result            = $shorten( $basket_name_array, 1, $to_trim );
				$to_trim           = $result[ 1 ];
				$basket_name_array = $result[ 0 ];
			}
			//then trim the site name first, if that's not enough move the array index and also trim the language
			for ( $i = 0; $i <= 1; $i ++ ) {
				if ( $to_trim > 0 ) {
					$result            = $shorten( $basket_name_array, 0, $to_trim );
					$to_trim           = $result[ 1 ];
					$basket_name_array = $result[ 0 ];
		...
		public static function get_basket_extra_fields_array($force_refresh = false) {
			if ($force_refresh) {
				$extra_fields = TranslationProxy::get_extra_fields_remote();
			} else {
				$extra_fields = TranslationProxy::get_extra_fields_local();
			}
			return $extra_fields;
