...
if ( !class_exists( 'TranslationProxy_Project' ) ) {
	class TranslationProxy_Project implements TranslationProxy_Project_Interface {
		...
		public static function generate_service_index( $service ) {
			...
			if ( isset( $service->id ) && isset( $service->custom_fields_data ) ) {
		 ...
		 */
		public function get_batch_job( $source_language = false, $target_languages = false ) {
		 ...
		 *
		 * @internal param bool|string $name
		 * @return mixed
		 */
		public function get_batch_job_id( $source_language = false, $target_languages = false ) {
			try {
		...
		public function commit_batch_job($tp_batch_id = false) {
			// doc:	http://git.icanlocalize.com/onthego/translation_proxy/wikis/commit_batch_job

			if ( ! $tp_batch_id ) {
				$tp_batch_id = $this->get_batch_job_id();
			}
