...
class WPML_Translation_Jobs_Table {
	...
	private function load_translation_jobs_collection() {
		$this->translation_jobs_collection = new WPML_Translation_Jobs_Collection( $this->translation_filter );
	}
	...
	public function get_paginated_jobs() {
		...
		foreach ( $paginated_results[ 'batches' ] as $batch_id => $batch ) {
			...
			if ( $paginated_results[ 'metrics' ][ $batch_id ][ 'display_from' ] > 1 ) {
				$paginated_results[ 'batches' ][ $batch_id ] = array_slice(
					$paginated_results[ 'batches' ][ $batch_id ],
					$paginated_results[ 'metrics' ][ $batch_id ][ 'display_from' ] - 1 );
			}
			...
			if ( $paginated_results[ 'metrics' ][ $batch_id ][ 'display_to' ] < $paginated_results[ 'metrics' ][ $batch_id ][ 'item_count' ] ) {
				$paginated_results[ 'batches' ][ $batch_id ] = array_slice(
					$paginated_results[ 'batches' ][ $batch_id ],
					0,
					$paginated_results[ 'metrics' ][ $batch_id ][ 'display_to' ] - $paginated_results[ 'metrics' ][ $batch_id ][ 'display_from' ] + 1 );
			}
		}
