	...
	Translation_Jobs.listing.views.ListingGroupView = Backbone.View.extend(
		{
			...
			render           : function (option) {
				...
				if (options.how_many < options.how_many_overall) {
					options.how_many = (self.model.get('display_from') ).toString() + '-' + (self.model.get('display_from') + options.how_many - 1 ).toString();
					options.show_out_of = '';
				}

				options.show_remaining = 'none';
				if (self.model.get('display_to') < options.how_many_overall) {
					options.show_remaining = '';
				}

				options.show_previous = 'none';
				if (self.model.get('display_from') > 1) {
				...
				if (url) {
					...
					self.model.set(
						'batch_name', batch_name_link(
							{
								name: self.model.get('batch_name'),
								url : url
							}
						)
					);
				...
				if (self.model.get('items')) {
					self.model.get('items').each(
						function (item_model) {
							...
							groups_view = new Translation_Jobs.listing.views.ListingItemView(options);
