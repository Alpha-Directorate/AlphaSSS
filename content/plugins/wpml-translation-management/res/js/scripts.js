...
jQuery(document).ready(function () {
	...
	icl_translation_jobs.find('td :checkbox').change(iclTmUpdateJobsSelection);
...
function icl_tm_assign_translator() {
	...
	translation_controls.find('.icl_tj_ok').unbind('click').click(function () {
		icl_tm_assign_translator_request(job_id, translator_id, this_translator);
	});
...
function icl_tm_assign_translator_request(job_id, translator_id, select) {
    ...
    function update_job_checkboxes() {
        var job_parent = jQuery('#icl-translation-jobs-basket');
	...
	function iclTmUpdateJobsSelection() {
		var translation_jobs = jQuery('#icl-translation-jobs');
		if (translation_jobs.find(':checkbox:checked').length > 0) {
			jQuery('#icl-tm-jobs-cancel-but').removeAttr('disabled');

			var checked_items = translation_jobs.find('th :checkbox');
			if (translation_jobs.find('td :checkbox:checked').length === translation_jobs.find('td :checkbox').length) {
				checked_items.attr('checked', 'checked');
			} else {
				checked_items.removeAttr('checked');
			}
